...
function boot(){
  ...
  .then(function(j){
    j.tree.forEach(function(f){
      if(/^(data|config|data-recent)\.json$|^(rss|sitemap)\.xml$|^(about|archive|mukhtarat|suwar)\.html$/.test(f.path))SHA[f.path]=f.sha});
...
function openF(e){
  ...
  mkTimeField();mkSelField();mkQuoteField();
  if($("fqs"))$("fqs").value="";
  setQuote(e&&e.q?e.q:"");
...
function drawFPrev(){
  ...
  $("fprev").innerHTML='<p class="pm">'+date+tp+rt+
    ...
    (QID?qCardHTML(QID,"javascript:void 0"):"")+
...
/* خانة الاقتباس — تُبنى من الجافاسكربت كأختيها، فلا نمسّ tahrir.html.
   تبحث في مداخلك بكلمة، فتختار المقتبَس منها ويُحفظ معرّفه في الحقل q. */
var QID = "";
function mkQuoteField(){
  if($("fqwrap"))return;
  var anchor=$("fsel");if(!anchor||!anchor.parentNode)return;
  var w=document.createElement("div");
  w.id="fqwrap";w.style.cssText="margin-top:14px";
  w.innerHTML=
    '<label style="display:block;margin-bottom:5px">الاقتباس من مدخل سابق</label>'+
    '<input id="fqs" placeholder="اكتب كلمة من نصّ المدخل القديم…" '+
      'style="font-family:inherit;font-size:14px;padding:7px 10px;border-radius:4px;width:100%">'+
    '<div id="fqres" style="display:none;max-height:190px;overflow:auto;margin-top:6px;'+
      'border:1px solid var(--rule);border-radius:6px"></div>'+
    '<div id="fqpick" style="display:none;margin-top:8px;padding:9px 11px;'+
      'background:var(--paper2);border:1px solid var(--rule);border-radius:6px;font-size:14px;line-height:1.85"></div>';
  anchor.parentNode.parentNode.insertBefore(w, anchor.parentNode.nextSibling);

  $("fqs").addEventListener("input",function(){
    var k=this.value.trim(), box=$("fqres");
    if(k.length<2){box.style.display="none";box.innerHTML="";return}
    var hits=DATA.filter(function(x){
      return x.id!==(cur&&cur.id) && x.t.indexOf(k)>=0 }).slice(0,12);
    if(!hits.length){box.style.display="block";
      box.innerHTML='<div style="padding:9px 11px;font-size:13px;opacity:.6">لا مدخل بهذه الكلمة.</div>';return}
    box.style.display="block";
    box.innerHTML=hits.map(function(x){
      return '<div class="fqit" data-id="'+esc(x.id)+'" style="padding:8px 11px;cursor:pointer;'+
        'border-bottom:1px solid var(--rule);font-size:13px;line-height:1.8">'+
        '<span style="opacity:.6">'+esc(x.d)+'</span><br>'+esc(snip(x.t,80))+"</div>"}).join("");
    Array.prototype.forEach.call(box.querySelectorAll(".fqit"),function(el){
      el.onclick=function(){setQuote(el.getAttribute("data-id"));
        box.style.display="none";$("fqs").value=""}})});
}

/* يضع المقتبَس أو يمحوه، ويعيد رسم المعاينة */
function setQuote(id){
  QID=id||"";
  var b=$("fqpick");if(!b)return;
  var t=null;
  if(QID)DATA.forEach(function(x){if(x.id===QID)t=x});
  if(!t){QID="";b.style.display="none";b.innerHTML="";drawFPrev();return}
  b.style.display="block";
  b.innerHTML='<span style="opacity:.6">'+esc(t.d)+"</span><br>"+esc(snip(t.t,120))+
    '<br><span class="link" id="fqdel" style="font-size:13px">إزالة الاقتباس</span>';
  $("fqdel").onclick=function(){setQuote("")};
  drawFPrev()}

/* بطاقة الاقتباس كما تظهر للقارئ — يستعملها المحرّر والصفحة المستقلّة */
function qCardHTML(id, href){
  var t=null;DATA.forEach(function(x){if(x.id===id)t=x});
  if(!t)return "";
  var im="";
  (t.m||[]).some(function(m){
    var f=m.f||"";
    if(f && !/\.(mp4|webm|mov|m4v)$/i.test(f)){
      im='<img class="qi" loading="lazy" src="'+CFG.media.base+"/"+xe(f)+'" alt="">';return true}
    return false});
  return '<a class="qc" href="'+href+'">'+
    '<span class="qb"><span class="qm">'+xe(t.d)+"</span>"+
    '<span class="qt">'+xe(snip(t.t,200))+"</span></span>"+im+"</a>"}
...
$("save").onclick=function(){
  ...
  if(cur){Object.keys(o).forEach(function(k){cur[k]=o[k]});
    if(!o.draft)delete cur.draft; if(!o.pin)delete cur.pin; if(!o.q)delete cur.q}
...
function pvMast(){
  ...
  var av=s.showAvatar!==false&&s.portrait
    ? '<img src="'+esc(s.portrait)+'" style="width:'+(s.avatarSize||66)+"px;height:"+(s.avatarSize||66)+
      'px;border-radius:12px;object-fit:cover;display:block;margin:11px auto 5px;border:2px solid '+L.gold+'">'+
...
/* ---------- أدوات المزامنة ---------- */
/* ---------- صفحتا التعريف والأرشيف ---------- */
function rootPage(o){
  var S="script",tok=(CFG.analytics&&CFG.analytics.cloudflareToken)||"";
  ...
  var img=CFG.site.portrait?(/^https?:/.test(CFG.site.portrait)?CFG.site.portrait:BASEURL+CFG.site.portrait):BASEURL+"card.jpg";
  return '<!DOCTYPE html>\n<html lang="ar" dir="rtl">\n<head>\n<meta charset="utf-8">\n'+
   '<meta name="viewport" content="width=device-width, initial-scale=1">\n<title>'+o.title+"</title>\n"+
   '<meta name="description" content="'+o.desc+'">\n<link rel="canonical" href="'+BASEURL+o.file+'">\n'+
   '<meta property="og:type" content="website">\n<meta property="og:site_name" content="'+xe(CFG.site.name)+'">\n'+
   '<meta property="og:title" content="'+o.title+'">\n<meta property="og:description" content="'+o.desc+'">\n'+
   '<meta property="og:url" content="'+BASEURL+o.file+'">\n<meta property="og:image" content="'+xe(img)+'">\n'+
   ...
   o.foot+"\n</footer>\n"+
   '<div class="sup" id="sup"><button data-go="top" aria-label="إلى أعلى الصفحة" title="إلى الأعلى">'+UPIC+
   '</button><button data-go="end" aria-label="إلى أسفل الصفحة" title="إلى الأسفل">'+DNIC+"</button></div>\n"+
   supjs+beacon+"\n</body>\n</html>\n"}

function buildAbout(){
  var s=CFG.site;
  var bio=CFG.doors.map(function(d){
    return '<span class="subj">'+xe(d.subj)+"</span>"+
      (d.pred?' <span class="pred">'+xe(d.pred)+"</span>":"")
  }).join(' <span class="sep">|</span> ');
  var body='<main class="about">'+
    (s.portrait?'<img src="'+xe(s.portrait)+'" alt="'+xe(s.name)+'" width="190" height="190">':"")+
    '<p class="lead">'+xe(s.about)+"</p>"+
    '<div class="fl" aria-hidden="true"><i></i><span class="lz"></span><i></i></div>'+
    '<p class="bio">'+bio+"</p>"+
    '<p class="sig">'+xe(s.tagline)+"</p>"+
    '<p class="meta2">'+
    (s.contactUrl?'للتواصل: <a href="'+xe(s.contactUrl)+'" target="_blank" rel="noopener">'+xe(s.contactLabel)+"</a><br>":"")+
    xe(s.location||"")+"</p>"+msgBox()+"</main>";
...
/* صندوق الرسائل — نموذجٌ في صفحة التعريف وحدها.
   يُرسل إلى Worker في حساب Cloudflare الخاصّ بك، ولا يمرّ بوسيط.
   يُطفأ بجعل contact.enabled = false في config.json، أو بإفراغ endpoint. */
function msgBox(){
  var c=CFG.contact||{};
  if(!c.enabled||!c.endpoint)return "";
  var S="script";
  var ts=c.turnstileKey
    ? '<div class="cf-turnstile" data-sitekey="'+xe(c.turnstileKey)+'" data-language="ar"></div>'+
      "<"+S+' src="https://challenges.cloudflare.com/turnstile/v0/api.js" async defer></'+S+">"
    : "";
  var h='<section class="msgbox" id="msg">'+
    "<h2>"+xe(c.title||"راسِلْني")+"</h2>"+
    (c.note?'<p class="mnote">'+xe(c.note)+"</p>":"")+
    '<div class="mfields">'+
    '<label for="m_n">اسمك</label>'+
    '<input id="m_n" type="text" maxlength="80" autocomplete="name">'+
    '<label for="m_r">بريدك أو حسابك، ليصلك الردّ</label>'+
    '<input id="m_r" type="text" maxlength="120" autocomplete="email" inputmode="email">'+
    '<label for="m_t">رسالتك</label>'+
    '<textarea id="m_t" rows="7" maxlength="4000"></textarea>'+
    '<div class="mhp" aria-hidden="true"><label for="m_h">اتركه فارغاً</label>'+
      '<input id="m_h" type="text" tabindex="-1" autocomplete="off"></div>'+
    ts+
    '<button type="button" id="m_go">إرسال</button>'+
    '<p class="mres" id="m_res" role="status" aria-live="polite"></p>'+
    "</div></section>";
  var js="<"+S+">(function(){var E="+JSON.stringify(c.endpoint)+",T0=Date.now(),"+
    "g=function(i){return document.getElementById(i)};var b=g('m_go'),r=g('m_res');if(!b)return;"+
    "b.addEventListener('click',function(){"+
    "var t=g('m_t').value.trim();"+
    "if(t.length<5){r.className='mres err';r.textContent='اكتب رسالتك أوّلاً.';return}"+
    "var tk='';try{var q=document.querySelector('[name=cf-turnstile-response]');if(q)tk=q.value}catch(e){}"+
    "b.disabled=true;r.className='mres';r.textContent='جارٍ الإرسال…';"+
    "fetch(E,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({"+
    "name:g('m_n').value,reply:g('m_r').value,text:t,hp:g('m_h').value,t:Date.now()-T0,cf:tk})})"+
    ".then(function(x){return x.json().catch(function(){return{ok:false,m:'تعذّر الاتصال.'}})})"+
    ".then(function(j){if(j.ok){g('m_t').value='';g('m_n').value='';g('m_r').value='';"+
    "r.className='mres ok';r.textContent=j.m||'وصلت رسالتك.'}"+
    "else{b.disabled=false;r.className='mres err';r.textContent=j.m||'تعذّر الإرسال.'}})"+
    ".catch(function(){b.disabled=false;r.className='mres err';"+
    "r.textContent='تعذّر الاتصال، حاول بعد قليل.'})});})();</"+S+">";
  return h+js}
...
/* ألبوم الصور — كل وسيط يقود إلى صفحة مدخله */
function buildGallery(){
  var pub=DATA.filter(function(e){return !e.draft&&(e.m||[]).length}),by={},order=[];
  pub.forEach(function(e){var y=e.iso.slice(0,4);if(!by[y]){by[y]=[];order.push(y)}by[y].push(e)});
  order.sort().reverse();
  var n=0;
  var body='<main class="gal">'+order.map(function(y){
    var cells=by[y].map(function(e){
      var alt=xe(snip(e.t,90));
      return (e.m||[]).map(function(m){
        n++;
        var thumb=CFG.media.base+"/"+xe(m.f);
        return '<a href="p/'+e.id+'.html" title="'+alt+'">'+
          '<img loading="lazy" src="'+thumb+'" alt="'+alt+'">'+
          (m.v?'<span class="vt">▶</span>':"")+"</a>"}).join("")}).join("");
    return '<h2 class="yr">'+AR(y)+"<span>"+AR(by[y].length)+" مدخلاً</span></h2>"+
      '<div class="grid">'+cells+"</div>"}).join("")+"</main>";
  return rootPage({file:"suwar.html",title:"ألبوم الصور · "+xe(CFG.site.name),
    desc:AR(n)+" صورة ومقطعاً من أرشيف "+xe(CFG.site.name)+" — تقود كلٌّ منها إلى نصّها",
    body:body,
    foot:'<a href="archive.html">الأرشيف</a> · <a href="mukhtarat.html">مختارات</a> · '+
         '<a href="about.html">عن الكاتب</a><br>'+xe(CFG.site.name)})}

function buildArchive(){
  var pub=DATA.filter(function(e){return !e.draft}),by={},order=[];
  pub.forEach(function(e){var y=e.iso.slice(0,4);if(!by[y]){by[y]=[];order.push(y)}by[y].push(e)});
  order.sort().reverse();
  var body='<main class="arch">'+order.map(function(y){
    return "<h2>"+AR(y)+"<span>"+AR(by[y].length)+"</span></h2><ul>"+
      by[y].map(function(e){
        return '<li><a href="p/'+e.id+'.html"><span class="ad">'+xe(e.d)+"</span>"+
          xe(snip(e.t,72))+"</a></li>"}).join("")+"</ul>"}).join("")+"</main>";
  return rootPage({file:"archive.html",title:"الأرشيف الزمني · "+xe(CFG.site.name),
  ...
  return api("/git/ref/heads/"+BRANCH).then(function(r){return r.json()}).then(function(ref){
    ...
    return api("/git/trees/"+baseTree+"?recursive=1").then(function(r){return r.json()})
      .then(function(tr){
        ...
        entries.push({path:"suwar.html",mode:"100644",type:"blob",content:buildGallery()});
...
function buildSitemap(){
  ...
  return '<?xml version="1.0" encoding="UTF-8"?>\n<urlset xmlns="http://www.sitemaps.org/schemas/sitemap/0.9">'+
    ...
    "<url><loc>"+BASEURL+"suwar.html</loc><priority>0.7</priority></url>"+
...
function commitData(msg,el){
  ...
  put("data.json",DATA,msg).then(function(){
    ...
    return putRaw("suwar.html",buildGallery(),"تحديث الألبوم")
  }).then(function(){
...
$("savecfg").onclick=function(){
  ...
  put("config.json",CFG,"تحديث الإعدادات").then(function(){
    ...
    return putRaw("about.html",buildAbout(),"تحديث صفحة التعريف")
  }).then(function(){
